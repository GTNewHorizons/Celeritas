...
public class ChunkTrackerImpl implements ChunkTracker, ClientChunkEventListener {
     ...
     */
    public ChunkTrackerImpl(int requiredNeighborRadius) {
    ...
    public void setFastMode(boolean fastMode) {
        this.setRequiredNeighborRadius(fastMode ? 0 : 1);
    }

    @Override
    public boolean isFastModeEnabled() {
        return this.requiredNeighborRadius == 0;
    }

    @Override
    public void forEachReady(RenderSectionManager sectionManager) {
        var iterator = this.chunkReady.iterator();

        while (iterator.hasNext()) {
            long pos = iterator.nextLong();
            sectionManager.onChunkAdded(PositionUtil.unpackChunkX(pos), PositionUtil.unpackChunkZ(pos));
        }
    }

    @Override
    public void forEachEvent(RenderSectionManager sectionManager) {
        var unloadIterator = this.unloadQueue.iterator();

        while (unloadIterator.hasNext()) {
            long pos = unloadIterator.nextLong();
            sectionManager.onChunkRemoved(PositionUtil.unpackChunkX(pos), PositionUtil.unpackChunkZ(pos));
        }

        this.unloadQueue.clear();

        var loadIterator = this.loadQueue.iterator();

        while (loadIterator.hasNext()) {
            long pos = loadIterator.nextLong();
            sectionManager.onChunkAdded(PositionUtil.unpackChunkX(pos), PositionUtil.unpackChunkZ(pos));
        }
