...
public abstract class PassThroughRenderSystem implements RenderSystemService {
    ...
    public static void initializeProfileDetection() {
        ...
        // Check if we have OpenGL 3.1+ and whether ARB_compatibility is present
        boolean hasGL31 = LWJGL.isOpenGLVersionSupported(3, 1);
        boolean hasGL32 = LWJGL.isOpenGLVersionSupported(3, 2);

        if (hasGL32 || hasGL31) {
            // OpenGL 3.1+ - check for ARB_compatibility extension
            // If we have the compatibility extension, we're in compatibility mode
            isCompatibilityProfile = LWJGL.isExtensionSupported(GLExtension.ARB_compatibility);
        } else {
    ...
    public void glUniformMatrix3(int location, boolean transpose, FloatBuffer value) {
        LWJGL.glUniformMatrix3fv(location, transpose, value);
    }

    @Override
    public void glUniformMatrix4(int location, boolean transpose, FloatBuffer value) {
        LWJGL.glUniformMatrix4fv(location, transpose, value);
    }
    ...
    public void setProjectionMatrixOrth(Matrix4f projectionMatrix) {
        ...
        if (isCompatibilityProfile()) {
            LWJGL.glMatrixMode(GL11.GL_PROJECTION);
            projectionMatrix.get(0, PROJECTION_MATRIX_BUFFER);
            LWJGL.glLoadMatrixf(PROJECTION_MATRIX_BUFFER);
            LWJGL.glMatrixMode(GL11.GL_MODELVIEW);
        }
    }
