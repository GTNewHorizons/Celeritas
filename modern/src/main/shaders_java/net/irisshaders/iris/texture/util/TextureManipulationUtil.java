...
public class TextureManipulationUtil {
	...
	public static void fillWithColor(int textureId, int maxLevel, int rgba) {
		...
		for (int level = 0; level <= maxLevel; ++level) {
			int width = GL_STATE_MANAGER.glGetTexLevelParameter(GL11.GL_TEXTURE_2D, level, GL11.GL_TEXTURE_WIDTH);
			int height = GL_STATE_MANAGER.glGetTexLevelParameter(GL11.GL_TEXTURE_2D, level, GL11.GL_TEXTURE_HEIGHT);
